...
export default function ExtrasSellersSlide({ data }: { data: MonthlyPresentationData }) {
  const items = (data.extras_sellers_ranking ?? []).slice(0, 3);
  ...
  return (
    <ScaledSlide>
      <div className="w-full h-full flex flex-col px-32 py-20">
        <p className="text-2xl font-mono uppercase tracking-[0.3em] text-primary/80 mb-4">Ranking de extras</p>
        <h2 className="text-7xl font-bold mb-12 flex items-center gap-4">
          <Sparkles className="w-16 h-16 text-primary" /> Quem mais vendeu serviços extras
        </h2>
          ...
          {order.map((p, i) => {
            ...
            return (
              <div key={i} className={`rounded-3xl border-2 p-8 ${colors[rank]} ${heights[rank]} flex flex-col justify-end`}>
                ...
                <p className="text-2xl text-muted-foreground mt-3">{fmtInt(p.qty)} extras</p>
                <p className="text-4xl font-bold text-success mt-3">{fmtBRL(p.revenue)}</p>
              </div>
            );
          })}
        </div>
        {items.length === 0 && <div className="text-center text-3xl text-muted-foreground mt-10">Sem vendas de extras no período.</div>}
      </div>
    </ScaledSlide>
  );
}
