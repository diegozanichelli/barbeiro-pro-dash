...
export default function SubscriptionEarningsForm() {
  ...
  const fetchEarnings = async () => {
    ...
    if (data) setEarnings(data as SubscriptionEarning[]);
  };
  ...
  const handleSave = async () => {
    if (!selectedBarber || !amount) {
      toast({
        title: "Campos obrigatórios",
        description: "Selecione o barbeiro e informe o valor.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (editingId) {
        ...
        // Verificar se já existe registro para este barbeiro/mês/ano
  ...
  const handleEdit = (earning: SubscriptionEarning) => {
    ...
    setAmount(earning.amount.toString());
