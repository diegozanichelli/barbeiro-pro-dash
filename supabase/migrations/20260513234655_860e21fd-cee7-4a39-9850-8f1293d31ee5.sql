  ...
  SELECT
    ...
    ROUND(
      ...
      * GREATEST(0, v_cal_passed - (
      ...
      / NULLIF(GREATEST(1, v_cal_total - (
          SELECT COUNT(*)::int FROM (
            SELECT date FROM hol_total
            UNION
            SELECT at3.date FROM abs_total at3 WHERE at3.barber_id = ob.id
          ) st2
        )), 0)::numeric,
    ...
    ROUND(
      ...
      / NULLIF(GREATEST(1, v_cal_total - (
          SELECT COUNT(*)::int FROM (
            SELECT date FROM hol_total
            UNION
            SELECT at4.date FROM abs_total at4 WHERE at4.barber_id = ob.id
          ) st3
        )), 0)::numeric,
