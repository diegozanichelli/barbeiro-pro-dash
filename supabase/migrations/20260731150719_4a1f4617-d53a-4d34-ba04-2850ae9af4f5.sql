CREATE OR REPLACE FUNCTION public.get_barber_report_range(p_barber_id uuid, p_start date, p_end date, p_unit_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  WITH tx AS (
    SELECT t.*,
      CASE
        ...
        ELSE 'service'
      END AS cat,
      -- Chave de atendimento: cliente + dia (Manaus). Sem telefone, cai no lançamento individual.
      CASE
        WHEN COALESCE(t.mobile_phone, '') <> ''
          THEN t.mobile_phone || ':' || ((t.created_at AT TIME ZONE 'America/Manaus')::date)::text
        ELSE 'anon:' || t.id::text
      END AS visit_key
  ...
  totals AS (
    SELECT
      ...
      -- Regra do projeto: assinatura não gera comissão operacional.
      COALESCE(SUM(commission_amount) FILTER (WHERE cat <> 'subscription'), 0) AS commission,
      COALESCE(SUM(commission_amount) FILTER (WHERE cat = 'subscription'), 0) AS subscription_commission,
  ...
  client_totals AS (
    SELECT COALESCE(NULLIF(t.mobile_phone, ''), '__sem_telefone__') AS phone_key,
      ...
      COUNT(DISTINCT t.visit_key) AS visits,
