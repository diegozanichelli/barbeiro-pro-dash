...
-- 1) Colunas opcionais para overrides por período
ALTER TABLE public.presentation_slide_overrides
  ADD COLUMN IF NOT EXISTS period_start date,
  ADD COLUMN IF NOT EXISTS period_end date;

CREATE INDEX IF NOT EXISTS idx_pres_overrides_period
  ON public.presentation_slide_overrides (organization_id, month, year, unit_key, period_start, period_end);

-- 2) Nova RPC com período flexível
CREATE OR REPLACE FUNCTION public.get_presentation_data_range(
  p_period_start  date,
  p_period_end    date,
  p_compare_start date,
  p_compare_end   date,
  p_unit_id       uuid DEFAULT NULL,
  p_target_ratio  numeric DEFAULT 1.0
)
RETURNS jsonb
LANGUAGE plpgsql
...
DECLARE
  v_org uuid;
  v_first date := p_period_start;
  v_last  date := p_period_end;
  v_prev_first date := p_compare_start;
  v_prev_last  date := p_compare_end;
  v_ratio numeric := COALESCE(NULLIF(p_target_ratio, 0), 1.0);

  v_cur_month int := EXTRACT(MONTH FROM v_first)::int;
  v_cur_year  int := EXTRACT(YEAR  FROM v_first)::int;
  v_prev_month int := EXTRACT(MONTH FROM v_prev_first)::int;
  v_prev_year  int := EXTRACT(YEAR  FROM v_prev_first)::int;
  v_next_month int;
  v_next_year  int;
...
BEGIN
  ...
  IF v_cur_month = 12 THEN
    v_next_month := 1; v_next_year := v_cur_year + 1;
  ELSE
    v_next_month := v_cur_month + 1; v_next_year := v_cur_year;
  END IF;
  ...
  -- Metas batidas (meta proporcional)
  WITH comms AS (
  ...
  goals AS (
    SELECT mg.barber_id,
           (mg.target_commission * v_ratio) AS target_commission,
           b.name, u.name AS unit_name
    FROM monthly_goals mg
    INNER JOIN barbers b ON b.id = mg.barber_id
    LEFT JOIN units u ON u.id = b.unit_id
    WHERE mg.organization_id = v_org
      AND mg.month = v_cur_month AND mg.year = v_cur_year
      AND (p_unit_id IS NULL OR b.unit_id = p_unit_id)
      AND b.status = 'active'
  ),
  joined AS (
  ...
  -- Ranking top 10 (meta proporcional)
  WITH dp AS (
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'target', COALESCE(mg.target_commission, 0) * v_ratio,
    'percent', CASE WHEN COALESCE(mg.target_commission,0) > 0
                    THEN ROUND(dp.commission*100/(mg.target_commission * v_ratio),1) ELSE 0 END
  ...
  -- Performance por unidade (meta proporcional)
  WITH dp AS (
  ...
  targets AS (
    SELECT b.unit_id, SUM(mg.target_commission) * v_ratio AS unit_target
  ...
  new_subs AS (
    ...
    FROM sale_transactions
    WHERE organization_id = v_org
      AND item_type = 'subscription' AND subscription_action = 'new'
      AND is_new_client = true
      AND mobile_phone IS NOT NULL AND mobile_phone <> ''
      AND source = 'manager'
      AND (created_at AT TIME ZONE 'America/Manaus')::date BETWEEN v_first AND v_last
  ...
  SELECT jsonb_build_object(
    ...
    'top_downgrade_reasons', COALESCE((
      ...
      FROM (
        ...
        FROM sale_transactions
        WHERE organization_id = v_org
          AND item_type = 'subscription' AND subscription_action = 'downgrade'
          AND source = 'manager'
          AND (created_at AT TIME ZONE 'America/Manaus')::date BETWEEN v_first AND v_last
          AND (p_unit_id IS NULL OR unit_id = p_unit_id)
  ...
  FROM sale_transactions
  WHERE organization_id = v_org
    AND item_type = 'subscription' AND source = 'manager'
    AND (created_at AT TIME ZONE 'America/Manaus')::date BETWEEN v_first AND v_last
    AND (p_unit_id IS NULL OR unit_id = p_unit_id);
  ...
  FROM (
    ...
    FROM sale_transactions st INNER JOIN barbers b ON b.id = st.barber_id
    ...
    GROUP BY b.name ORDER BY total DESC LIMIT 3
  ) t;
  ...
  WITH by_day AS (
    SELECT d.date,
      ...
      SUM(d.clients_count) AS clients
    FROM daily_productions d INNER JOIN barbers b ON b.id = d.barber_id
    WHERE d.organization_id = v_org
      AND d.date BETWEEN v_first AND v_last
      AND (p_unit_id IS NULL OR b.unit_id = p_unit_id)
    GROUP BY d.date ORDER BY revenue DESC LIMIT 1
  ),
  star AS (
    ...
    WHERE d.organization_id = v_org
      ...
      AND (p_unit_id IS NULL OR b.unit_id = p_unit_id)
    GROUP BY b.name ORDER BY commission DESC LIMIT 1
  )
  SELECT jsonb_build_object(
  ...
  -- Alertas (< 85% da meta proporcional)
  WITH comms AS (
    SELECT d.barber_id, SUM(d.commission_earned) AS earned
    FROM daily_productions d INNER JOIN barbers b ON b.id = d.barber_id
    WHERE d.organization_id = v_org
  ...
  underperf AS (
    SELECT b.name, u.name AS unit_name, COALESCE(c.earned,0) AS earned,
           (mg.target_commission * v_ratio) AS target_commission,
           ROUND(COALESCE(c.earned,0)*100/NULLIF(mg.target_commission * v_ratio,0),1) AS percent
    FROM monthly_goals mg
    INNER JOIN barbers b ON b.id = mg.barber_id
    LEFT JOIN units u ON u.id = b.unit_id
    ...
    WHERE mg.organization_id = v_org
      ...
      AND COALESCE(c.earned,0) < (mg.target_commission * v_ratio) * 0.85
  )
  SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY (t).percent ASC), '[]'::jsonb)
  INTO v_alerts FROM underperf t;

  -- Meta total do próximo mês (sempre o mês seguinte ao período)
  SELECT COALESCE(SUM(mg.target_commission), 0)
  INTO v_next_target
  FROM monthly_goals mg INNER JOIN barbers b ON b.id = mg.barber_id
  WHERE mg.organization_id = v_org
    AND mg.month = v_next_month AND mg.year = v_next_year
    AND (p_unit_id IS NULL OR b.unit_id = p_unit_id)
    AND b.status = 'active';
    ...
    SELECT d.barber_id, b.name,
      SUM(
        CASE
          ...
          ELSE COALESCE(d.services_total,0)+COALESCE(d.products_total,0)
        END
      ) AS revenue, SUM(d.clients_count) AS clients
    FROM daily_productions d INNER JOIN barbers b ON b.id = d.barber_id
    WHERE d.organization_id = v_org
      AND d.date BETWEEN v_first AND v_last
      AND (p_unit_id IS NULL OR b.unit_id = p_unit_id)
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'barber_id', c.barber_id, 'name', c.name,
    'revenue_curr', c.revenue, 'revenue_prev', COALESCE(p.revenue, 0),
    'delta_pct', CASE WHEN COALESCE(p.revenue,0) > 0
                      THEN ROUND((c.revenue - p.revenue) * 100.0 / p.revenue, 1) ELSE NULL END,
    'clients_curr', c.clients, 'clients_prev', COALESCE(p.clients, 0),
  ...
  FROM (
    ...
    FROM sale_transactions st
    WHERE st.organization_id = v_org AND st.source='manager' AND st.item_type='service'
      AND (st.created_at AT TIME ZONE 'America/Manaus')::date BETWEEN v_first AND v_last
      AND (p_unit_id IS NULL OR st.unit_id = p_unit_id)
    GROUP BY st.item_name ORDER BY qty DESC LIMIT 5
  ) t;
  ...
  FROM (
    SELECT st.item_name AS name, COUNT(*) AS qty, COALESCE(SUM(st.price_sold),0) AS revenue
    FROM sale_transactions st
    WHERE st.organization_id = v_org AND st.source='manager' AND st.item_type='product'
      AND (st.created_at AT TIME ZONE 'America/Manaus')::date BETWEEN v_first AND v_last
      AND (p_unit_id IS NULL OR st.unit_id = p_unit_id)
    GROUP BY st.item_name ORDER BY qty DESC LIMIT 5
  ) t;
  ...
  SELECT jsonb_build_object(
    ...
    'pct_curr', CASE WHEN (SELECT COUNT(*) FROM cur_clients) > 0
                     THEN ROUND((SELECT COUNT(*) FROM cur_with_extra)::numeric * 100 / (SELECT COUNT(*) FROM cur_clients), 1) ELSE 0 END,
    'pct_prev', CASE WHEN (SELECT COUNT(*) FROM prev_clients) > 0
                     THEN ROUND((SELECT COUNT(*) FROM prev_with_extra)::numeric * 100 / (SELECT COUNT(*) FROM prev_clients), 1) ELSE 0 END
  ) INTO v_extras_penetration;

  -- Ranking vendedores de produtos
  SELECT COALESCE(jsonb_agg(jsonb_build_object('barber_name', name, 'qty', qty, 'revenue', revenue) ORDER BY revenue DESC), '[]'::jsonb)
  ...
  -- Vendas da recepção
  SELECT COALESCE(jsonb_agg(jsonb_build_object('unit_name', unit_name, 'revenue', revenue, 'count', cnt) ORDER BY revenue DESC), '[]'::jsonb)
  INTO v_reception_sales
  FROM (
    SELECT COALESCE(u.name, 'Sem unidade') AS unit_name, COUNT(*) AS cnt, COALESCE(SUM(st.price_sold), 0) AS revenue
    FROM sale_transactions st LEFT JOIN units u ON u.id = st.unit_id
    WHERE st.organization_id = v_org AND st.source='manager' AND st.barber_id IS NULL
      AND (st.created_at AT TIME ZONE 'America/Manaus')::date BETWEEN v_first AND v_last
      AND (p_unit_id IS NULL OR st.unit_id = p_unit_id)
    GROUP BY u.name
  ) t;
  ...
  WITH cur_new AS (
    ...
    WHERE organization_id = v_org AND source='manager' AND is_new_client = true
      AND mobile_phone IS NOT NULL AND mobile_phone <> ''
      AND (created_at AT TIME ZONE 'America/Manaus')::date BETWEEN v_first AND v_last
      AND (p_unit_id IS NULL OR unit_id = p_unit_id)
  ),
  cur_ret AS (
    SELECT COUNT(DISTINCT mobile_phone) AS total FROM sale_transactions
    WHERE organization_id = v_org AND source='manager'
      AND COALESCE(is_new_client, false) = false AND item_type = 'service'
      AND mobile_phone IS NOT NULL AND mobile_phone <> ''
      AND (created_at AT TIME ZONE 'America/Manaus')::date BETWEEN v_first AND v_last
      AND (p_unit_id IS NULL OR unit_id = p_unit_id)
  ),
  ...
  SELECT jsonb_build_object(
    'new_curr', (SELECT total FROM cur_new), 'returning_curr', (SELECT total FROM cur_ret),
    'new_prev', (SELECT total FROM prev_new), 'returning_prev', (SELECT total FROM prev_ret)
  ...
  WITH visits AS (
    ...
    FROM sale_transactions
    WHERE organization_id = v_org AND source='manager' AND item_type='service'
      AND mobile_phone IS NOT NULL AND mobile_phone <> ''
      AND (created_at AT TIME ZONE 'America/Manaus')::date BETWEEN v_first AND v_last
      AND (p_unit_id IS NULL OR unit_id = p_unit_id)
  ...
  SELECT jsonb_build_object(
    ...
    'avg_visits_per_client', CASE WHEN COUNT(DISTINCT mobile_phone) > 0
                                  THEN ROUND(COUNT(*)::numeric / COUNT(DISTINCT mobile_phone), 2) ELSE 0 END
  ) INTO v_visit_frequency FROM visits;

  -- Top vendedores assinatura
  SELECT COALESCE(jsonb_agg(jsonb_build_object('barber_name', name, 'new_subs_qty', qty, 'mrr_generated', mrr) ORDER BY qty DESC), '[]'::jsonb)
  INTO v_top_subscription_sellers
  FROM (
    SELECT b.name, COUNT(*) AS qty, COALESCE(SUM(st.price_sold), 0) AS mrr
    FROM sale_transactions st INNER JOIN barbers b ON b.id = st.barber_id
    WHERE st.organization_id = v_org AND st.source='manager'
      AND st.item_type='subscription' AND st.subscription_action='new'
      AND (st.created_at AT TIME ZONE 'America/Manaus')::date BETWEEN v_first AND v_last
      AND (p_unit_id IS NULL OR st.unit_id = p_unit_id)
    GROUP BY b.name ORDER BY qty DESC LIMIT 5
  ) t;

  -- Metas do período anterior (mesma lógica de proporcionalidade)
  WITH comms_prev AS (
    SELECT d.barber_id, SUM(d.commission_earned) AS earned
    FROM daily_productions d INNER JOIN barbers b ON b.id = d.barber_id
    WHERE d.organization_id = v_org
  ...
  goals_prev AS (
    SELECT mg.barber_id, (mg.target_commission * v_ratio) AS target_commission, b.name, u.name AS unit_name
    FROM monthly_goals mg
    INNER JOIN barbers b ON b.id = mg.barber_id
    LEFT JOIN units u ON u.id = b.unit_id
    WHERE mg.organization_id = v_org
      AND mg.month = v_prev_month AND mg.year = v_prev_year
      AND (p_unit_id IS NULL OR b.unit_id = p_unit_id)
      AND b.status = 'active'
      AND mg.target_commission > 0
  ),
  ...
  SELECT jsonb_build_object(
    'hit', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('barber', name, 'unit', unit_name, 'pct', pct, 'earned', earned, 'target', target_commission) ORDER BY pct DESC)
      FROM joined WHERE earned >= target_commission
    ), '[]'::jsonb),
    'missed', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('barber', name, 'unit', unit_name, 'pct', pct, 'earned', earned, 'target', target_commission) ORDER BY pct ASC)
      FROM joined WHERE earned < target_commission
    ), '[]'::jsonb)
  ...
  WITH big_ticket AS (
    ...
    FROM sale_transactions st LEFT JOIN barbers b ON b.id = st.barber_id
    WHERE st.organization_id = v_org AND st.source='manager'
      AND (st.created_at AT TIME ZONE 'America/Manaus')::date BETWEEN v_first AND v_last
      AND (p_unit_id IS NULL OR st.unit_id = p_unit_id)
  ...
  top_barber AS (
    ...
    WHERE d.organization_id = v_org
      AND d.date BETWEEN v_first AND v_last
      AND (p_unit_id IS NULL OR b.unit_id = p_unit_id)
    GROUP BY b.name ORDER BY revenue DESC LIMIT 1
  ),
  best_streak_calc AS (
    SELECT b.name AS barber, d.date,
  ...
  best_streak AS (
    SELECT barber, COUNT(*) AS days FROM best_streak_calc
    GROUP BY barber, grp ORDER BY days DESC LIMIT 1
  )
  SELECT jsonb_build_object(
  ...
  RETURN jsonb_build_object(
    'org_name', v_org_name,
    'month', v_cur_month, 'year', v_cur_year,
    'period_start', v_first, 'period_end', v_last,
    'compare_start', v_prev_first, 'compare_end', v_prev_last,
    'target_ratio', v_ratio,
    'kpis', v_kpis,
