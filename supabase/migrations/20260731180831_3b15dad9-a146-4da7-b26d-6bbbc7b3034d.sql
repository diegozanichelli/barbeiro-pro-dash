CREATE OR REPLACE FUNCTION public.get_audit_finding_rows(p_check_key text, p_start date, p_end date, p_unit_id uuid DEFAULT NULL, p_limit integer DEFAULT 200, p_offset integer DEFAULT 0, p_organization_id uuid DEFAULT NULL)
    ...
    SELECT COALESCE(jsonb_agg(r), '[]'::jsonb) INTO v_rows FROM (
      SELECT jsonb_build_object(
               ...
               'clients', jsonb_agg(jsonb_build_object('id', c.id, 'name', c.name, 'phone', c.mobile_phone,
                 'created_at', c.created_at, 'plan_id', c.subscription_plan_id,
                 'unit_id', c.subscription_unit_id, 'unit_name', cu.name) ORDER BY c.created_at)
             ) AS r
      FROM public.clients c
      LEFT JOIN public.units cu ON cu.id = c.subscription_unit_id
      WHERE c.organization_id = v_org AND COALESCE(c.mobile_phone, '') <> ''
      GROUP BY c.mobile_phone
      HAVING count(*) > 1
        AND (p_unit_id IS NULL OR bool_or(c.subscription_unit_id = p_unit_id))
...
CREATE OR REPLACE FUNCTION public.get_report_audit_findings(p_start date, p_end date, p_unit_id uuid DEFAULT NULL, p_organization_id uuid DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public AS $fn$
DECLARE
  v_org uuid;
  v_today date := (now() AT TIME ZONE 'America/Manaus')::date;
