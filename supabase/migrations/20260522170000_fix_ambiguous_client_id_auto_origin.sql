-- Fix ambiguous column references in PL/pgSQL function suggest_client_origin_units
-- that can surface as: "client_id is ambiguous" when called via apply_auto_origin_units.
...
BEGIN
  ...
  ranked_barber AS (
    SELECT bc.client_id, bc.unit_id, bc.cnt,
           ROW_NUMBER() OVER (PARTITION BY bc.client_id ORDER BY bc.cnt DESC) AS rn
    FROM barber_counts bc
  ),
  primary_barber AS (
    SELECT rb.client_id, rb.unit_id, rb.cnt FROM ranked_barber rb WHERE rb.rn = 1
  ),
  last_sale AS (
